...
function generateAllElements() {
  ...
  for(let a = 0; a < totalRingElements; a++){
    ...
    for (let i = 1; i <= totalCircleElements; i++) {
      ...
      let animElement = document.createElement('a-animation');
      animElement.setAttribute("attribute", "material.color");
      animElement.setAttribute("from", '#' + getRandomColor());
      animElement.setAttribute("to", '#' + getRandomColor());
      animElement.setAttribute("dur", 250);
      animElement.setAttribute("repeat", "indefinite");

      circleElement.appendChild(animElement);      
      ...
      let xyz = document.getElementsByTagName("a-light")[0];
      xyz.setAttribute(`alongpath`, `curve: .track${a}; dur: ${pathDuration}; loop: true`);
